...
from configs.settings import (
    ...
    RISK_PROFILES, INITIAL_BALANCE, CONFIDENCE_THRESHOLD,
)
...
def run_backtest(symbol: str, model_name: str, risk_profile_name: str):
    risk = RISK_PROFILES[risk_profile_name]

    model_path = os.path.join(MODELS_DIR, f"{symbol}_{model_name}.pkl")
    features_path = os.path.join(MODELS_DIR, f"{symbol}_feature_cols.pkl")
    ...
    for i in range(len(test_df)):
        ...
        conf_ratio = min(max(conf_ratio, 0.1), 1.0)

        leverage = max(1, int(risk["max_leverage"] * conf_ratio))
        position_pct = risk["max_position_pct"] * conf_ratio
    ...
    stats = compute_stats(closed_trades, equity_curve, symbol, model_name, risk_profile_name)
    return stats


def compute_stats(trades, equity_curve, symbol, model_name, risk_profile_name):
    if not trades:
        return {
            "symbol": symbol,
            "model": model_name,
    ...
    for t in trades:
        if hasattr(t.entry_time, 'timestamp') and hasattr(t.exit_time, 'timestamp'):
            try:
                ...
                holding_bars.append(diff)
            except Exception:
                holding_bars.append(0)
    ...
    return {
        ...
        "avg_holding_hours": round(np.mean(holding_bars), 1) if holding_bars else 0,
...
def run_all_backtests():
    ...
    for symbol in SYMBOLS:
        for model_name in ["lightgbm", "xgboost"]:
            for risk_name in RISK_PROFILES.keys():
                print(f"Backtesting {symbol} | {model_name} | {risk_name}...")
                stats = run_backtest(symbol, model_name, risk_name)
                if stats:
...
if __name__ == "__main__":
    ...
    print("\n" + "="*80)
    print("FULL BACKTEST RESULTS (sorted by profit %)")
    print("="*80)
